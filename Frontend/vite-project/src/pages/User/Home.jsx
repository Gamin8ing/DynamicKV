...
import { User, ShoppingBag, Heart, Bell, Moon, Sun, CreditCard, Compass, ChevronUp, Star, Mail, Phone, MapPin, X } from 'lucide-react';
...
const Home = () => {
  ...
  const stats = [
    ...
    { title: "Wishlist", value: "12", icon: <Heart size={20} /> },
  ...
  // Featured electronic items (inspired by the UI layout of the reference image)
  ...
  const notifications = [
    ...
    { id: 2, title: "New arrivals in your wishlist", time: "Yesterday", read: true },
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-blue-50 to-teal-50'}`}>
                ...
                <div className={`absolute right-0 mt-2 w-72 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border shadow-lg rounded-lg overflow-hidden z-20`}>
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.map(notification => (
                      <div 
                        key={notification.id}
                        className={`p-3 border-b ${isDarkMode ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-100 hover:bg-gray-50'} ${!notification.read ? (isDarkMode ? 'bg-gray-700/50' : 'bg-blue-50') : ''}`}
                      >
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        {/* Featured Items Section (Electronics, inspired by the UI layout of the reference image) */}
        ...
        {/* Stats grid */}
        <div id="stats-section" className={`grid grid-cols-2 md:grid-cols-4 gap-4 mb-12 transition-all duration-700 ${animatedElements.stats ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      <div className={`fixed bottom-0 left-0 right-0 ${isDarkMode ? 'bg-gray-800 bg-opacity-80 border-gray-700' : 'bg-white bg-opacity-80 border-gray-200'} backdrop-blur-md border-t py-2 px-6 z-10`}>
        <div className="flex justify-between max-w-md mx-auto">
          ...
          <Link to="/wishlist">
            <button 
              className={`flex flex-col items-center p-2 ${activeTab === 'wishlist' ? 'text-cyan-600' : isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
      ...
      {/* Mobile navigation */}
      <div className={`fixed bottom-0 left-0 right-0 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-t shadow-lg sm:hidden z-10`}>
        <div className="flex justify-around">
          <Link to="/">
            <button 
              className={`flex flex-col items-center py-3 flex-1 ${activeTab === 'dashboard' ? 'text-cyan-500' : isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
              onClick={() => setActiveTab('dashboard')}
            >
              <Compass size={20} />
          ...
          <Link to="/orders">
            <button 
              className={`flex flex-col items-center py-3 flex-1 ${activeTab === 'orders' ? 'text-cyan-500' : isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
              onClick={() => setActiveTab('orders')}
            >
              <ShoppingBag size={20} />
              <span className="text-xs mt-1">Orders</span>
          ...
          <Link to="/wishlist">
            <button 
              className={`flex flex-col items-center py-3 flex-1 ${activeTab === 'wishlist' ? 'text-cyan-500' : isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
              onClick={() => setActiveTab('wishlist')}
            >
              <Heart size={20} />
              <span className="text-xs mt-1">Wishlist</span>
          ...
          <Link to="/profile">
            <button 
              className={`flex flex-col items-center py-3 flex-1 ${activeTab === 'profile' ? 'text-cyan-500' : isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
              onClick={() => setActiveTab('profile')}
            >
              <User size={20} />
              <span className="text-xs mt-1">Profile</span>
